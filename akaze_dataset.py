    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            if descriptors is None:
                descriptors = np.zeros((max_descriptors, 61))  # AKAZE descriptor length is typically 61 bytes (adjust as needed)

            # Flatten the descriptors to a 1D array (N * 61 where N is the number of keypoints)
...
np.savetxt('preprocess/features_akaze.csv', features_array, delimiter=',')